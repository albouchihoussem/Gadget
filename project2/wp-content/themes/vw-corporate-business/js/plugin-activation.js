jQuery(document).ready(function($) {
    'use strict';
    var this_obj = vw_corporate_business_plugin_activate_plugin;

    $('#gutenberg_editor .plugin-activation-redirect a').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.vw-corporate-business-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message vw-corporate-business-plugin-install');
                button.addClass('button-primary vw-corporate-business-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.vw-corporate-business-recommended-plugins .vw-corporate-business-activation-note').css('display','block');
    ...
    $(document).on('click', '.vw-corporate-business-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .vw-corporate-business-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .vw-corporate-business-recommended-plugins').css('display','none');
    });
