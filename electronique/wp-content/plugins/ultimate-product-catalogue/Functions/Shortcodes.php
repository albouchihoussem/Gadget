...
function Insert_Product_Catalog($atts) {
	...
	if (isset($_GET['overview_mode'])) {$overview_mode = sanitize_text_field( $_GET['overview_mode'] );}
	...
	$CatalogueItems = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $catalogue_items_table_name WHERE Catalogue_ID=%d ORDER BY Position", $Catalogue->Catalogue_ID ) );
	...
	$HeaderBar .= "<input type='hidden' id='upcp-selected-categories' name='categories' value='" . esc_attr( $_POST['categories'] ) . "' />";
	$HeaderBar .= "<input type='hidden' id='upcp-selected-subcategories' name='sub-categories' value='" . esc_attr( $_POST['sub-categories'] ) . "' />";
	$HeaderBar .= "<input type='hidden' id='upcp-selected-tags' name='tags' value='" . esc_attr( $_POST['tags'] ) . "' />";
	$HeaderBar .= "<input type='hidden' id='upcp-selected-prod-name' name='prod_name' value='" . esc_attr( $_POST['prod_name'] ) . "' />";
	$HeaderBar .= "<input type='hidden' id='upcp-selected-current-page' name='current_page' value='" . esc_attr( $_POST['current_page'] ) . "' />";
	$HeaderBar .= "<input type='hidden' id='upcp-selected-current-url' name='current_url' value='" . esc_attr( get_permalink() ) . "' />";
	$HeaderBar .= "</form>";
	...
	if ($Lightbox_Mode == "Yes") {
		...
		$CatalogAreaTopString .= "<div id='upcp-lightbox-close-div'" . (is_admin_bar_showing() ? ' class="extra-margin"' : '') . ">x</div>";
	...
	if (isset($_GET['categories'])) {$category = explode(",", sanitize_text_field( $_GET['categories'] ) );}
	elseif (isset($_POST['categories']) and $_POST['categories'] != "") {$category = explode(",", sanitize_text_field( $_POST['categories'] ) );}
	elseif ($category == "") {$category = array();}
	else {$category = explode(",", $category);}
	if (isset($_GET['sub-categories'])) {$subcategory = explode(",", sanitize_text_field( $_GET['sub-categories'] ) );}
	elseif (isset($_POST['sub-categories']) and $_POST['sub-categories'] != "") {$subcategory = explode(",", sanitize_text_field( $_POST['sub-categories'] ) );}
	elseif ($subcategory == "") {$subcategory = array();}
	else {$subcategory = explode(",", $subcategory);}
	if (isset($_GET['tags'])) {$tags = explode(",", sanitize_text_field( $_GET['tags'] ) );}
	elseif (isset($_POST['tags']) and $_POST['tags'] != "") {$tags = explode(",", sanitize_text_field( $_POST['tags'] ) );}
	elseif ($tags == "") {$tags = array();}
	else {$tags = explode(",", $tags);}
	if (isset($_POST['current_page']) and $_POST['current_page'] != "") {$current_page = sanitize_text_field( $_POST['current_page'] );}

	if (isset($_POST['prod_name']) and $_POST['prod_name'] != "") {$prod_name = sanitize_text_field( $_POST['prod_name'] );}
	...
	if ($custom_fields != "") {
		...
		foreach ($Custom_Field_IDs as $Custom_Field_ID){
			$Field_ID = intval( substr( $Custom_Field_ID, 0, strpos( $Custom_Field_ID, "=" ) ) );
			$Field_Value = esc_sql( substr( $Custom_Field_ID, strpos( $Custom_Field_ID, "=" ) + 1 ) );
	...
	foreach ($CatalogueItems as $CatalogueItem) {
		...
		if ($CatalogueItem->Item_ID != "" and $CatalogueItem->Item_ID != 0) {
			...
			$ProdTagObj = $wpdb->get_results( $wpdb->prepare( "SELECT Tag_ID FROM $tagged_items_table_name WHERE Item_ID=%d", $CatalogueItem->Item_ID ) );
			//if ($ajax_reload == "No") {
				$Prod_Custom_Fields = $wpdb->get_results( $wpdb->prepare( "SELECT Field_ID, Meta_Value FROM $fields_meta_table_name WHERE Item_ID=%d", $Product->Get_Item_ID() ) );
			//}
		...
		if ($CatalogueItem->Category_ID != "" and $CatalogueItem->Category_ID != 0) {
			if ((sizeOf($category) == 0 or in_array($CatalogueItem->Category_ID, $category)) and $Category_Heading_Style != "None") {
				$CatProdCount = 0;
				$Category = $wpdb->get_row( $wpdb->prepare( "SELECT Category_Name, Category_Description, Category_Image FROM $categories_table_name WHERE Category_ID=%d", $CatalogueItem->Category_ID ) );
			...
			$Item_IDs = $wpdb->get_results( $wpdb->prepare( "SELECT Item_ID FROM $items_table_name WHERE Category_ID=%d ORDER BY Item_Category_Product_Order", $CatalogueItem->Category_ID ) );

			foreach ($Item_IDs as $Item_ID) {
				$Product = new UPCP_Product(array("ID" => $Item_ID->Item_ID));
				$ProdTagObj = $wpdb->get_results( $wpdb->prepare( "SELECT Tag_ID FROM $tagged_items_table_name WHERE Item_ID=%d", $Product->Get_Item_ID() ) );
				//if ($ajax_reload == "No") {
					$Prod_Custom_Fields = $wpdb->get_results( $wpdb->prepare("SELECT Field_ID, Meta_Value FROM $fields_meta_table_name WHERE Item_ID=%d", $Product->Get_Item_ID() ) );
				//}
		...
		if ($CatalogueItem->SubCategory_ID != "" and $CatalogueItem->SubCategory_ID != 0) {
			if (sizeOf($subcategory) == 0 or in_array($CatalogueItem->SubCategory_ID, $subcategory)) {
				$Products = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $items_table_name WHERE SubCategory_ID=%d", $CatalogueItem->SubCategory_ID ) );

				foreach ($Products as $Product) {
					$ProdTagObj = $wpdb->get_results( $wpdb->preare( "SELECT Tag_ID FROM $tagged_items_table_name WHERE Item_ID=%d", $CatalogueItem->Item_ID ) );
					//if ($ajax_reload == "No") {
						$Prod_Custom_Fields = $wpdb->get_results( $wpdb->prepare( "SELECT Field_ID, Meta_Value FROM $fields_meta_table_name WHERE Item_ID=%d", $Product->Item_ID ) );
					//}
	...
	$ProdCatString .= implode( ',', array_filter( array_keys( $ProdCats ) ) );

	$ProdSubCatString .= implode( ',', array_filter( array_keys( $ProdSubCats ) ) );

	$ProdTagString .= implode( ',', array_filter( array_keys( $ProdTags ) ) );

	$ProdCustomFieldsString .= implode( ',', array_filter( array_keys( $ProdCustomFields ) ) );
	...
	if (isset($_GET['Product_ID'])) {$Bottom_JS .= "jQuery(window).load(OpenProduct('" . intval( $_GET['Product_ID'] ) . "'));";}
	$Bottom_JS .= "</script>";
...
function Insert_Minimal_Products($atts) {
	...
	// If there's a product selected, return that product
	if ( (get_query_var( 'single_product' ) != "" or ( isset( $_GET['SingleProduct'] ) and $_GET['SingleProduct'] != "" ) ) and $catalogue_url == "" ) {
	...
	if ($product_ids != "") {
		...
		array_walk( $Product_Array, 'intval' );
		$Products_String = implode( ',', $Product_Array );
		$Products = $wpdb->get_results( "SELECT * FROM $items_table_name WHERE Item_ID IN (" . esc_sql( $Products_String ) . ")" );
	}
	elseif ($catalogue_id != "") {
		$Item_IDs = "'999999',";
		$Category_IDs = "'999999',";
		$SubCategory_IDs = "'999999',";
		$Catalogue_Items = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $catalogue_items_table_name WHERE Catalogue_ID=%d", $catalogue_id ) );
	...
	elseif ($category_id != "") {
		$Products = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $items_table_name WHERE  Category_ID=%d AND Item_Display_Status!='Hide' ORDER BY rand() LIMIT %s", $category_id, $product_count ) );
	}
	elseif ($subcategory_id != "") {
		$Products = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $items_table_name WHERE  SubCategory_ID=%d AND Item_Display_Status!='Hide' ORDER BY rand() LIMIT %s", $subcategory_id, $product_count ) );
	}
	else {
		$Products = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $items_table_name WHERE Item_Date_Created!='0000-00-00 00:00:00' AND Item_Display_Status!='Hide' ORDER BY Item_Date_Created ASC LIMIT %s", $product_count ) );
	}
	foreach ($Products as $Product) {
...
function UPCP_AddProduct($format, $Product_Object, $Tags, $AjaxReload = "No", $AjaxURL = "") {
	...
	$Item_Images = $wpdb->get_results( $wpdb->prepare( "SELECT Item_Image_URL, Item_Image_ID FROM $item_images_table_name WHERE Item_ID=%d ORDER BY Item_Image_Order", $Product_Object->Get_Item_ID() ) );
	$TagsString = "";
	...
	if ($format == "Thumbnail") {
		...
		$thumbnailTags = $wpdb->get_results( $wpdb->prepare( "SELECT Tag_ID FROM $tagged_items_table_name WHERE Item_ID=%d", $Product_Object->Get_Item_ID() ) );
		if (is_array($thumbnailTags)) {
			$thumbnailTagsString = '';
			foreach ($thumbnailTags as $thumbnailTag) {
				$thumbnailTagName = $wpdb->get_var( $wpdb->prepare( "SELECT Tag_Name FROM $tags_table_name WHERE Tag_ID=%d", $thumbnailTag->Tag_ID ) );
				$thumbnailTagsString .= $thumbnailTagName . ", ";
			}
		}
		...
		$ProductString .= "</div>\n";
		$ProductString .= apply_filters('upcp_price_div', "<div id='prod-cat-price-" . $Product_Object->Get_Item_ID() . "' class='prod-cat-price upcp-thumb-price' data-item_price='" . $Item_Price . "'>" . $Item_Display_Price . "</div>", array('Item_ID' => $Product_Object->Get_Item_ID(), 'Item_Price' => $Item_Price, 'Layout' => $format)) . "\n";
	...
	if ($format == "List") {
		...
		$ProductString .= "<div id='prod-cat-price-" . $Product_Object->Get_Item_ID() . "' class='prod-cat-price upcp-list-price' data-item_price='" . $Item_Price . "' onclick='ToggleItem(" . $Product_Object->Get_Item_ID() . ");'>" . $Item_Display_Price. "</div>\n";
	...
	if ($format == "Detail") {
		...
		$ProductString .= apply_filters('upcp_price_div', "<div id='prod-cat-price-" . $Product_Object->Get_Item_ID() . "' class='prod-cat-price upcp-detail-price' data-item_price='" . $Item_Price . "'>" . $Item_Display_Price . "</div>", array('Item_ID' => $Product_Object->Get_Item_ID(), 'Item_Price' => $Item_Price, 'Layout' => $format)) . "\n";
...
function SingleProductPage() {
	...
	else {$Product = $wpdb->get_row($wpdb->prepare("SELECT * FROM $items_table_name WHERE Item_ID='%d'", intval( $_GET['SingleProduct'] ) ));}
	$Item_Images = $wpdb->get_results( $wpdb->prepare( "SELECT Item_Image_URL, Item_Image_ID FROM $item_images_table_name WHERE Item_ID=%d ORDER BY Item_Image_Order", $Product->Item_ID ) );
	$Item_Videos = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $item_videos_table_name WHERE Item_ID=%d ORDER BY Item_Video_Order ASC", $Product->Item_ID ) );
	...
	if (is_array($Tags)) {
		foreach ($Tags as $Tag) {
			$TagInfo = $wpdb->get_row( $wpdb->prepare( "SELECT Tag_Name FROM $tags_table_name WHERE Tag_ID=%d", $Tag->Tag_ID ) );
	...
	$uri_parts = explode('?', $_SERVER['REQUEST_URI'], 2);
	$uri_parts[1] = isset( $uri_parts[1] ) ? $uri_parts[1] : '';
	...
	if ($Custom_Product_Page == "No") {
		...
		$ProductString .= "<div id='prod-cat-addt-details-main-images-" . $Product->Item_ID . "' class='prod-cat-addt-details-main-image-div'>";
			...
			foreach ($Item_Images as $Image) {
				$Slide_Counter++; 
				$ProductString .= "<a class='upcp-thumb-anchor " . $Lightbox_Class . " href='" . htmlspecialchars($Image->Item_Image_URL, ENT_QUOTES) ."' data-ulbsource='" . htmlspecialchars($Image->Item_Image_URL, ENT_QUOTES) . "' data-ulbtitle='" . htmlspecialchars( isset( $Image->Title ) ? $Image->Title : '', ENT_QUOTES) . "' data-ulbdescription='" . htmlspecialchars( isset( $Image->Caption ) ? $Image->Caption : '', ENT_QUOTES) . "'>";
				$ProductString .= "<img src='" . htmlspecialchars($Image->Item_Image_URL, ENT_QUOTES) . "' id='prod-cat-addt-details-thumb-". $Image->Item_Image_ID . "' class='prod-cat-addt-details-thumb " . $Disable_Lightbox . "' onclick='ZoomImage(\"" . $Product->Item_ID . "\", \"" . $Image->Item_Image_ID . "\"); return false;'>";
				$ProductString .= "</a>";
			}
			$ProductString .= "</div>";
		...
		if (in_array("Category", $Extra_Elements)) {$ProductString .= "<div class='prod-category-container upcp-product-side-container'>\n<div class='upcp-side-title'>" . $Additional_Info_Category_Label . " </div>" . $Product->Category_Name . "</div>";}
			...
			if (in_array("CustomFields", $Extra_Elements)) {
				...
				$Fields = $wpdb->get_results("SELECT Field_Name, Field_ID, Field_Type, Field_Slug FROM $fields_table_name ORDER BY Field_Sidebar_Order");
				foreach ($Fields as $Field) {
					$Value = $wpdb->get_row( $wpdb->prepare( "SELECT Meta_Value FROM $fields_meta_table_name WHERE Item_ID=%d and Field_ID=%d", $Product->Item_ID, $Field->Field_ID ) );
					if ($Custom_Fields_Blank != "Yes" or $Value->Meta_Value != "") {
						if (is_object($Value)) { $Meta_Value = UPCP_Decode_CF_Commas($Value->Meta_Value);}
						else {$Meta_Value = "";}
						$Meta_Value = apply_filters('upcp_cf_'.$Field->Field_Slug.'_display', $Meta_Value, $Field);
						...
						elseif ($Field->Field_Type == "link") {
							...
							$Meta_Value = UPCP_CF_Link_Decode($Meta_Value);
							$Meta_Value->text = empty($Meta_Value->text) ? $Meta_Value->url : $Meta_Value->text;

							$ProductString .= "<div class='upcp-tab-title'>" . $Field->Field_Name . ":</div><span><a href='" . $Meta_Value->url . "'>". $Meta_Value->text ."</a></span><br>";
						}
	...
	elseif ($Custom_Product_Page == "Tabbed" || $Custom_Product_Page == "Shop_Style") {
		...
		$ProductString .= "<div id='upcp-tabbed-main-image-div-" . $Product->Item_ID . "' class='upcp-tabbed-main-image-div" . ($Lightbox != 'Yes' ? ' no-image-link' : '') . "'>";
		$ProductString .= "<div class='upcp-tabbed-main-image-inner'>";
		$ProductString .= "<div class='upcp-tabbed-image-container'>";
		$ProductString .= "<a href='" . $PhotoURL . "' class='" . ($Lightbox != 'Yes' ? 'no-image-link ' : '') . "prod-cat-addt-details-link-a prod-cat-pp-main-img-a " . $Lightbox_Clicker_Class . "' data-ulbsource='" . $PhotoURL . "'> ";
		$ProductString .= "  $PhotoCode " ;
		$ProductString .= "</a>";
		$ProductString .= "</div>";
		...
		foreach ($Item_Images as $Image) {
			...
			$ProductString .= "<a class='upcp-thumb-anchor " . $Lightbox_Class . "' href='" . htmlspecialchars($Image->Item_Image_URL, ENT_QUOTES) . "' data-ulbsource='" . htmlspecialchars($Image->Item_Image_URL, ENT_QUOTES) . "' data-ulbtitle='" . ( isset( $Image->Title ) ? htmlspecialchars($Image->Title, ENT_QUOTES) : '' ) . "' data-ulbdescription='" . ( isset( $Image->Caption ) ? htmlspecialchars($Image->Caption, ENT_QUOTES) : '' ) . "'>";
			$ProductString .= "<img src='" . htmlspecialchars($Image->Item_Image_URL, ENT_QUOTES) . "' id='prod-cat-addt-details-thumb-". $Image->Item_Image_ID . "' class='upcp-tabbed-addt-details-thumb " . $Disable_Lightbox . "' onclick='ZoomImage(\"" . $Product->Item_ID . "\", \"" . $Image->Item_Image_ID . "\"); return false;' >";
			$ProductString .= "</a></li>";
		}
		...
		if ( is_array($Tabs_Array) and ! empty($Tabs_Array) ) {
...
function BuildSidebar($category, $subcategory, $tags, $prod_name) {
	...
	$Date_Ascending_Label = $UPCP_Options->Get_Option("UPCP_Date_Ascending_Label");
	$Date_Descending_Label = $UPCP_Options->Get_Option("UPCP_Date_Descending_Label");
	$Product_Name_Search_Label = $UPCP_Options->Get_Option("UPCP_Product_Name_Search_Label");
	$Product_Search_Text_Label = $UPCP_Options->Get_Option("UPCP_Product_Name_Text_Label");
	...
	if ($Date_Ascending_Label != "") {$Date_Ascending_Text = $Date_Ascending_Label;}
	else {$Date_Ascending_Text = __('Newest', 'ultimate-product-catalogue');}
	if ($Date_Descending_Label != "") {$Date_Descending_Text = $Date_Descending_Label;}
	else {$Date_Descending_Text = __('Oldest', 'ultimate-product-catalogue');}
	if ($Product_Name_Search_Label != "") {$SearchLabel = $Product_Name_Search_Label;}
	else {
		if ($ProductSearch == "namedesc" or $ProductSearch == "namedesccust") {$SearchLabel = __("Product Search:", 'ultimate-product-catalogue');}
		else {$SearchLabel = __("Product Name:", 'ultimate-product-catalogue');}
	...
	if ($ProdCatString != "") {$Categories = $wpdb->get_results("SELECT Category_ID, Category_Name, Category_Image FROM $categories_table_name WHERE Category_ID IN (" . esc_sql( $ProdCatString ) . ") ORDER BY Category_Sidebar_Order");}
	if ($ProdSubCatString != "") {$SubCategories = $wpdb->get_results("SELECT SubCategory_ID, SubCategory_Name, SubCategory_Image, Category_ID FROM $subcategories_table_name WHERE SubCategory_ID IN (" . esc_sql( $ProdSubCatString ) . ") ORDER BY SubCategory_Sidebar_Order");}
	if ($ProdTagString != "") {$Tags = $wpdb->get_results("SELECT Tag_ID, Tag_Name FROM $tags_table_name WHERE Tag_ID IN (" . esc_sql( $ProdTagString ) . ") ORDER BY Tag_Sidebar_Order");}
	else {$Tags = array();}
	if ($ProdCustomFieldsString != "") {$Custom_Fields = $wpdb->get_results("SELECT Field_ID, Field_Name, Field_Control_Type FROM $fields_table_name WHERE Field_ID IN (" . esc_sql( $ProdCustomFieldsString ) . ") AND Field_Searchable='Yes' ORDER BY Field_Sidebar_Order");}
	...
	foreach ($Sidebar_Items_Order as $Sidebar_Item) {
		...
		if ($Sidebar_Item == "Product Sort" and $Full_Version == "Yes" and !empty($Product_Sort)) {
			...
			if (in_array("Date", $Product_Sort)) {
				$SortString .= "<option value='date_asc'>" . $Date_Ascending_Text . "</option>";
				$SortString .= "<option value='date_desc'>" . $Date_Descending_Text . "</option>";
			}
			$SortString .= "</select>";
			$SortString .= "</div>";
			$SortString .= "</div>";
		...
		if ($Sidebar_Item == "Product Search" and $ProductSearch != "none") {
			...
			$SearchString .= "<input type='text' id='upcp-name-search' class='jquery-prod-name-text' name='Text_Search' placeholder='" . $Product_Name_Text . "' onfocus='FieldFocus(this);' onblur='FieldBlur(this);' onkeyup='UPCP_DisplayPage(\"1\");'>\n";
		...
		if ( $Sidebar_Item == "Categories" and ! empty($Categories) ) {
		...
		if ($Sidebar_Item == "Sub-Categories" and ! empty($SubCategories) and $Sidebar_Order != "Hierarchical") {
		...
		if ($Sidebar_Item == "Tags" and ! empty($Tags) ) {
		...
		if ($Sidebar_Item == "Custom Fields" and sizeOf($Custom_Fields) > 0) {
			...
			foreach ($Custom_Fields as $Custom_Field) {
				if (is_array($ProdCustomFields[$Custom_Field->Field_ID])) {
					...
					$ProdCustomFields[$Custom_Field->Field_ID] = apply_filters( 'upcp_cf_label_display_sort' , $ProdCustomFields[$Custom_Field->Field_ID]);
		...
		foreach ($Gridster as $Element) {
			switch ($Element->element_class) {
				...
				case "text":
					...
					break;
				case "custom_field":
					$Field = $wpdb->get_row( $wpdb->prepare( "SELECT Field_Name, Field_Type FROM $fields_table_name WHERE Field_ID=%d", $Element->element_id ) );
					$Field_Value = $wpdb->get_row( $wpdb->prepare( "SELECT Meta_Value FROM $fields_meta_table_name WHERE Field_ID=%d AND Item_ID=%d", $Element->element_id, $Product->Item_ID ) );
					...
					elseif ($Field->Field_Type == "link") {
						$Meta_Value = UPCP_CF_Link_Decode($Meta_Value);
						$Meta_Value->text = empty($Meta_Value->text) ? $Meta_Value->url : $Meta_Value->text;
						$ProductString .= "<a href='" . $Meta_Value->url . "'>" . $Meta_Value->text . "</a>";
					}
					else {if(is_object($Field_Value)){$ProductString .= do_shortcode($Meta_Value);}}
					break;
				case "custom_label":
					$Field = $wpdb->get_row( $wpdb->prepare( "SELECT Field_Name FROM $fields_table_name WHERE Field_ID=%d", $Element->element_id ) );
...
function Get_Next_Previous($Product, $Next_Previous_Type = "Manual") {
	...
	elseif ($Next_Previous_Type == "Auto") {
		$Next_Product_ID = $wpdb->get_var( $wpdb->prepare( "SELECT Item_ID FROM $items_table_name WHERE Item_ID>%d ORDER BY Item_ID ASC LIMIT 1", $Product->Item_ID ) );
		$Previous_Product_ID = $wpdb->get_var( $wpdb->prepare( "SELECT Item_ID FROM $items_table_name WHERE Item_ID<%d ORDER BY Item_ID DESC LIMIT 1", $Product->Item_ID ) );
	}

	$Next_Product = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $items_table_name WHERE Item_ID=%d", $Next_Product_ID ) );
	$Previous_Product = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $items_table_name WHERE Item_ID=%d", $Previous_Product_ID ) );
...
function UPCP_Get_Related_Products($Product, $Related_Type = "Auto") {
	...
	if ($Related_Type == "Manual") {
		...
		foreach ($Related_Products_IDs as $Related_Product_ID) {$ID_String .= "'" . intval( $Related_Product_ID ) . "',";}
	...
	elseif ($Related_Type == "Auto") {
		...
		$Sub_Category_Products = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $items_table_name WHERE SubCategory_ID=%d AND Item_ID!=%d AND Item_Display_Status='Show'", $Product->SubCategory_ID, $Product->Item_ID ), ARRAY_A);
		if ($wpdb->num_rows < 5) {
			$Category_Products = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $items_table_name WHERE Category_ID=%d AND SubCategory_ID!=%d AND Item_ID!=%d AND Item_Display_Status='Show'", $Product->Category_ID, $Product->SubCategory_ID, $Product->Item_ID ), ARRAY_A);
		}
...
function UPCP_Add_Product_Reviews($Product) {
	...
	$ReturnString .= "<h2>" . __("Leave a review", 'ultimate-product-catalogue') . "</h2>";
...
function Order_Products($Product, $Related_Products_Array) {
	...
	$Product_Tags = $wpdb->get_results( $wpdb->prepare( "SELECT Tag_ID FROM $tagged_items_table_name WHERE Item_ID=%d", $Product->Item_ID), ARRAY_A);

	foreach ($Related_Products_Array as $key => $Related_Product) {
		$Related_Product_Tags = $wpdb->get_results( $wpdb->prepare( "SELECT Tag_ID FROM $tagged_items_table_name WHERE Item_ID=%d", $Related_Product['Item_ID']), ARRAY_A);
...
function UPCP_Product_Comparison($Products = array(), $omit_fields) {
	...
	foreach ($Products as $Product_ID) {
		$Product = new UPCP_Product( array( 'ID' => intval( $Product_ID ) ) );
		...
		foreach ($Fields as $Field) {
			if ($Field->Field_Display_Comparison != "No" and !in_array($Field->Field_Name, $Omit_Fields_Array)) {
				...
				if ($Field->Field_Type == "link") {
					$Meta_Value = UPCP_CF_Link_Decode($Product->Get_Custom_Field_By_ID($Field->Field_ID));
					$Meta_Value->text = empty($Meta_Value->text) ? $Meta_Value->url : $Meta_Value->text;
					$ReturnString .= "<div class='upcp-product-comparison-custom-field-value upcp-pc-value'><a href='" . $Meta_Value->url . "'>" . $Meta_Value->text . "</a></div>";
				}
...
function UPCP_Get_Catalog_Overview($Catalogue_Items, $Overview_Type, $Category_ID = 0) {
	...
	foreach ($Catalogue_Items as $CatalogueItem) {
		...
		elseif ($CatalogueItem->Category_ID != "" and $CatalogueItem->Category_ID != 0) {
			...
			elseif ($Category_ID == $CatalogueItem->Category_ID) {
				$Products = $wpdb->get_results( $wpdb->prepare( "SELECT Item_ID FROM $items_table_name WHERE Category_ID=%d", $CatalogueItem->Category_ID ) );
				foreach ($Products as $Product) {
		...
		elseif ($CatalogueItem->SubCategory_ID != "" and $CatalogueItem->SubCategory_ID != 0) {
			if ($Overview_Type == "Categories") {$Item_IDs[] = $wpdb->get_var( $wpdb->prepare( "SELECT Category_ID FROM $subcategories_table_name WHERE SubCategory_ID=%d", $CatalogueItem->SubCategory_ID ) );}
	...
	if ($Overview_Type == "Categories") {$Items = $wpdb->get_results("SELECT * FROM $categories_table_name WHERE Category_ID IN (" . esc_sql( $ID_String ) . ") ORDER BY Category_Sidebar_Order");}
	else {$Items = $wpdb->get_results("SELECT * FROM $subcategories_table_name WHERE SubCategory_ID IN (" . esc_sql( $ID_String ) . ") ORDER BY SubCategory_Sidebar_Order");}
...
function FilterCount($Product, $Tags, $Custom_Fields) {
	...
	if (is_array($Custom_Fields)) {
		foreach ($Custom_Fields as $Custom_Field) {
			...
			$Field_Type = $wpdb->get_var( $wpdb->prepare( "SELECT Field_Type FROM $fields_table_name WHERE Field_ID=%d", $Custom_Field->Field_ID ) );
...
function SearchProductName($Item_ID, $ProductName, $ProductDescription, $SearchName, $CaseInsensitive, $SearchLocation) {
	...
	if ($NameSearchMatch != "Yes" and $SearchLocation == "namedesccust") {
		...
		$Metas = $wpdb->get_results( $wpdb->prepare( "SELECT Meta_Value FROM $fields_meta_table_name WHERE Item_ID=%d and Meta_Value LIKE %s", $Item_ID, $SearchName) );
...
function Custom_Field_Check($Custom_Fields_Sql_String, $Custom_Field_Count, $Product_ID) {
	...
	$Fields = $wpdb->get_results( $wpdb->prepare( "SELECT Field_ID FROM $fields_meta_table_name WHERE " . $Custom_Fields_Sql_String . " AND Item_ID=%d", $Product_ID ) );
...
function ConvertCustomFields($Description, $Item_ID, $Item_Name = "") {
	...
	if (is_array($Fields)) {
		...
		foreach ($Fields as $Field) {
			...
			elseif ($Field->Field_Type == "link") {
				$Meta_Value = isset($MetaArray[$Field->Field_ID]) ? $MetaArray[$Field->Field_ID] : '';
				$Meta_Value = UPCP_CF_Link_Decode($Meta_Value);
				$Meta_Value->text = empty($Meta_Value->text) ? $Meta_Value->url : $Meta_Value->text;
						
				$LinkString = "<a href='" . $Meta_Value->url . "'>" . $Meta_Value->text . "</a>";
				$Description = str_replace("[" . $Field->Field_Slug . "]" , $LinkString, $Description);
			}
			else {
...
function AddCustomFields($ProductID, $Layout) {
	...
	$Fields = $wpdb->get_results( $wpdb->prepare( "SELECT Field_ID, Field_Name, Field_Type, Field_Slug FROM $fields_table_name WHERE Field_Displays=%s OR Field_Displays='both' ORDER BY Field_Sidebar_Order", $Layout ) );
	if (is_array($Fields)) {
		...
		foreach ($Fields as $Field) {
			$Meta = $wpdb->get_row( $wpdb->prepare( "SELECT Meta_Value FROM $fields_meta_table_name WHERE Field_ID=%d AND Item_ID=%d", $Field->Field_ID, $ProductID ) );
			...
			if ($Meta_Value != "" or $Custom_Fields_Blank != "Yes") {
				...
				if ($Field->Field_Type == "file") {
					...
					if('link' === $Field->Field_Type) {
						$Meta_Value = UPCP_CF_Link_Decode($Meta_Value);
						$text = empty($Meta_Value->text) ? $Field->Field_Name : $Meta_Value->text;
						$CustomFieldString .= $AddBreak . "<span class='upcp-cf-label'>&nbsp;</span><a class='upcp-cf-value' href='".$Meta_Value->url."'>" . $Field->Field_Name . "</a>";
					}
					else {
...
function UPCP_Single_Page_Inquiry_Form() {
	...
	if ($CF_7_Installed or $WP_Forms_Installed) {
		...
		if (isset($_POST['return_URL'])) {$ReturnString .= "<div class='upcp-inquire-cart-back-link'><a href='" . esc_attr( esc_url_raw( $_POST['return_URL'] ) ) . "' >" . __("Back to Catalog", 'ultimate-product-catalogue') . "</a></div>";}
		...
		if (isset($_COOKIE['upcp_cart_products'])) {
			...
			if (is_array($Products_Array)) {
				foreach ($Products_Array as $Product_ID) {
					$Product_Name = $wpdb->get_var($wpdb->prepare("SELECT Item_Name FROM $items_table_name WHERE Item_ID=%d", intval( $Product_ID ) ));
