...
if (!class_exists('UPCP_Product')){
    class UPCP_Product {
		...
		function __construct($params = array()) {
			...
			if (isset($params['ID'])) {
				$this->Item = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $items_table_name WHERE Item_ID=%d", intval( $params['ID'] ) ) );
			}
			...
			elseif (get_query_var('single_product') != "" or $_GET['SingleProduct'] != "") {	
				...
				else {$Item_ID = intval( $_GET['SingleProduct'] );}
    	...
    	function Get_Product_Tag_String() {
    		...
    		$Tags = $wpdb->get_results( $wpdb->prepare( "SELECT Tag_ID FROM $tagged_items_table_name WHERE Item_ID=%d", $this->Item->Item_ID ) );
			if (is_array($Tags)) {
				foreach ($Tags as $Tag) {
...
function UPCP_CF_Link_Decode($value)
{
  $tmp_Value = json_decode($value);
  if(JSON_ERROR_NONE != json_last_error()) {
    $tmp_Value = new stdClass();
    $tmp_Value->text = '';
    $tmp_Value->url = $value;
  }
