...
function UPCP_Add_SEO() {
	...
	if ($SEO_Option == "Yoast") {
		...
		add_action( 'wpseo_opengraph_image', 'UPCP_Add_OG_Image_new', 29 );
...
function UPCP_Yoast_Change_Canonical_URL( $str ) {
	...
	elseif ($_GET['SingleProduct'] != "") {return $str . "?SingleProduct=" . intval( $_GET['SingleProduct'] );}
	else {return $str;}
}
...
function UPCP_Yoast_Change_Description( $str ) {
	...
	if (get_query_var('single_product') == "" and array_key_exists('SingleProduct', $_GET) and $_GET['SingleProduct'] == "") {
		return $str;
	}

	if ($Pretty_Links == "Yes") {$SEO_Description = $wpdb->get_var( $wpdb->prepare( "SELECT Item_SEO_Description FROM $items_table_name WHERE Item_Slug=%s", trim(get_query_var('single_product'), "/? ") ) );}
	else {$SEO_Description = $wpdb->get_var( $wpdb->prepare( "SELECT Item_SEO_Description FROM $items_table_name WHERE Item_ID=%d", intval( $_GET['SingleProduct'] ) ) );}
...
function UPCP_Yoast_Change_Title( $str ) {
	...
	if ($Pretty_Links == "Yes") {$Title_Elements = $wpdb->get_row( $wpdb->prepare( "SELECT Item_Name, Category_Name, SubCategory_Name FROM $items_table_name WHERE Item_Slug=%s", trim(get_query_var('single_product'), "/? ") ) );}
	else {$Title_Elements = $wpdb->get_row( $wpdb->prepare( "SELECT Item_Name, Category_Name, SubCategory_Name FROM $items_table_name WHERE Item_ID=%d", intval( $_GET['SingleProduct'] ) ) );}
...
function UPCP_Add_OG_Image() {
	...
	if( ! array_key_exists('wpseo_og', $GLOBALS) ) return;

	$Pretty_Links = get_option("UPCP_Pretty_Links");

	if (get_query_var('single_product') == "" and $_GET['SingleProduct'] == "") {return;}

	if ($Pretty_Links == "Yes") {$Item_Photo_URL = $wpdb->get_var( $wpdb->prepare( "SELECT Item_Photo_URL FROM $items_table_name WHERE Item_Slug=%s", trim(get_query_var('single_product'), "/? ") ) );}
	else {$Item_Photo_URL = $wpdb->get_var( $wpdb->prepare( "SELECT Item_Photo_URL FROM $items_table_name WHERE Item_ID=%d", intval( $_GET['SingleProduct'] ) ) );}

	$GLOBALS['wpseo_og']->image_output($Item_Photo_URL);
}

function UPCP_Add_OG_Image_new() {
	global $wpdb, $items_table_name;
