=== Meta Store ===
Contributors: Mystical Themes
Tags: blog, e-commerce, one-column, two-columns, right-sidebar, footer-widgets, custom-menu, custom-logo, featured-images, threaded-comments, full-width-template, translation-ready
Requires at least: 5.6
Tested up to: 5.6
Stable tag: 1.0.3
License: GNU General Public License v2 or later
License URI: http://www.gnu.org/licenses/gpl-3.0.html

A Woocommerce based Ecommerce theme called Meta Store.

== Description ==
Meta Store is a beautifully designed responsive and fully customizable ecommerce template. It could be used for all types of e-commerce online stores, including Fashion, Clothing, Electronics, Furniture, Restaurant, Interior Store, Gadgets Store, Home Appliances Shop, Online shop, Organic Store, Retail Store, Automobile Shop, Books Store, Photo Store, Jewellery and almost any kind of store. The theme totally supports Elementor Plugin. It is fully responsive, translation ready, SEO friendly and compatible with WooCommerce. It has a user-friendly and powerful customizer panel which enhances the development of your site. Check Out Demo – https://demo.mysticalthemes.com/meta-store/

== Copyright ==
Meta Store is distributed under the terms of the GNU GPL

== Installation ==

1. In your admin panel, go to Appearance > Themes and click the Add New button.
2. Click Upload Theme and Choose File, then select the theme's .zip file. Click Install Now.
3. Click Activate to use your new theme right away.

== Frequently Asked Questions ==

= Does this theme support any plugins? =
Meta Store includes support for WooCommerce, Elementor, Meta Store Elements and WP Forms plugins.

== Changelog ==
= 1.0.6 - Mar 9 2021 =
* Made Compatible with Swift Demo Import Plugin.

= 1.0.5 - Mar 3 2021 =
* Fixed link underline issue and keyboard navigation

= 1.0.4 - Feb 19 2021 =
* Fixed Sanitization and security issues

= 1.0.3 - Jan 14 2021 =
* Minor Image Fixes

= 1.0.1 - Dec 15 2020 =
* Fixed Design and Security Isses

= 1.0.0 - June 25 2020 =
* Initial release

== Resources ==
* Based on Underscores https://underscores.me/, (C) 2012-2017 Automattic, Inc., [GPLv2 or later](https://www.gnu.org/licenses/gpl-2.0.html)

* JS Files
	Chosen, Patrick Filler
	License: MIT
	Source: https://github.com/harvesthq/chosen

	Color picker, Stefan Petre
	License: MIT
	Source: www.eyecon.ro

    select2, Kevin Brown, Igor Vaynberg
	License: MIT
	Source: https://select2.github.io

    wpColorPicker, kallookoo
	License: GPLv2
	Source: https://github.com/kallookoo/wp-color-picker-alpha

    jQuery Superfish Menu - Copyright (c) 2013 Joel Birch
    License: Dual licensed under the MIT and GPL licenses (http://opensource.org/licenses/MIT)
    Source: http://users.tpg.com.au/j_birch/plugins/superfish/

* Icons
	elegant icons, Rico Kaltofen
	License: GPLv3 & MIT
	Source: https://github.com/heimrichhannot/elegant-icons

	Icofonts, rtsinani
	License: MIT License
	Source: https://github.com/lugolabs/icofont

	MaterialDesign Icons, Templarian
	License: SIL Open Font License 1.1
	Source: https://materialdesignicons.com/

	Elementor Icons, kobizz
	License: SIL OFL 1.1
	Source: https://github.com/elementor/elementor-icons

* Fonts
	Varela Round, by Joe Prince
	License: Open Font License
	Source: https://fonts.google.com/specimen/Varela+Round

* PHP
	Breadcrumb Trail, Justin Tadlock
	License: GNU GPL
	Source: https://github.com/justintadlock/breadcrumb-trail

* Screenshot Images
	Pxhere Images
	License: CC0 1.0 Universal (CC0 1.0)
	Source: https://pxhere.com/en/photo/1234394
			https://pxhere.com/en/photo/1555961
			https://stocksnap.io/photo/camera-old-V97225MRV3
			https://picography.co/wireless-headphones-isolated/